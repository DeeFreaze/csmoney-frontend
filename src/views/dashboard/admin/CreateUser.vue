      ...
      <v-card-text>
        <template>
          <v-form v-model="valid" ref="form" lazy-validation>
            <v-text-field label="Full Name" v-model="steam_user_name" required></v-text-field>
...
export default class CreateUser extends Vue {
  ...
  public async submit() {
    if (await this.$validator.validateAll()) {
      ...
      if (this.steam_user_name) {
        updatedProfile.steam_user_name = this.steam_user_name;
      }
      if (this.email) {
