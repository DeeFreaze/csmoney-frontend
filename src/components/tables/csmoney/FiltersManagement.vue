<template>
  <div>
    ...
    <v-divider></v-divider>
    <v-container v-if="!!filtersArr.length" fluid>
      <v-row justify="center">
        <v-chip
          v-for="(filter, idx) in filtersArr"
          :key="idx"
          ...
          outlined
          @click="dialogChipFilterEdit(filter.chip_id)"
          @click:close="dialogFilterDelete(filter.chip_id)"
        >
          <v-icon left>article</v-icon>
          {{ filter.filter_title_form }}
        </v-chip>
      </v-row>
    </v-container>
  </div>
...
<script>
  ...
  import {
    getCSMoneyFilters, createCSMoneyFilter,
    updateCSMoneyFilter, deleteCSMoneyFilter
  } from '@/utils';
    ...
    methods: {
      async getFiltersHandler() {
        ...
        if (response.data) {
          ...
          for (let i = 0; i < this.filtersArr.length - 1; i++) {
            this.filtersArr[i].chip_id = i;
          }

          this.$emit('filters-arr', this.filtersArr);
        }
      },
      ...
      async dialogFilterEdit() {
        for (let arrayItem of this.filtersArr) {
          if (arrayItem.chip_id === this.chipID) {
            !this.filterTitle
      ...
      async dialogFilterDelete(chipID) {
        for (let arrayItem of this.filtersArr) {
          if (arrayItem.chip_id === chipID) {
            await this.deleteFilterHandler(arrayItem.id);
          }
        }
      },
      async dialogChipFilterEdit(chipID) {
        for (let arrayItem of this.filtersArr) {
          if (arrayItem.chip_id === chipID) {
            this.chipID = chipID;
            this.filterTitle = arrayItem.filter_title_form;
            this.nameChips = [].concat(arrayItem.name_chips_form);
            this.skinWear = [].concat(arrayItem.skin_wear);
            this.stickersChips = [].concat(arrayItem.stickers_chips_form);
            this.countSalesValue = arrayItem.count_sales_form;
            this.priceFromFilterValue = arrayItem.price_from_filter_form;
            this.priceToFilterValue = arrayItem.price_to_filter_form;
            this.priceHumanMinimumFromFilterValue =
              arrayItem.price_human_minimum_from_filter_form;
            this.priceHumanMinimumToFilterValue =
              arrayItem.price_human_minimum_to_filter_form;
            this.profitFromFilterValue = arrayItem.profit_from_filter_form;
            this.profitToFilterValue = arrayItem.profit_to_filter_form;
            this.floatFromFilterValue = arrayItem.float_from_filter_form;
            this.floatToFilterValue = arrayItem.float_to_filter_form;
            this.daysLeftFromFilterValue = arrayItem.days_left_from_filter_form;
            this.daysLeftToFilterValue = arrayItem.days_left_to_filter_form;
          }
        }
